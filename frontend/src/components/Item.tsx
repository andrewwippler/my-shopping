...
export default function Item({
  ...
  return (
    <>
      <div
        className={`${classInclude} flex items-center my-2 text-l text-blue-900`}
      >
        {editTag ? (
          <div>
            <form
            ...
            <button
              ...
              className="px-4 py-2 mt-2 font-semibold text-sm bg-red-300 hover:bg-red-500 text-white w-full shadow-sm inline-flex items-center"
            >
          ...
          <>
            <button
              onClick={() => socket.emit("check", item.id)}
              className="px-4 py-2 mr-2 font-semibold text-sm bg-blue-300 hover:bg-blue-500 text-white shadow-sm inline-flex items-center"
            >
              <CheckIcon className="h-4 w-4" />
            </button>
            <span className="grow">{item.name}</span>{" "}
            {/* , {item.list}, {String(item.picked)} */}
            <button
              onClick={() => setEditTag(true)}
              className="px-4  py-2 font-semibold text-sm bg-green-300 hover:bg-green-500 text-white rounded-full shadow-sm inline-flex items-center"
            >
